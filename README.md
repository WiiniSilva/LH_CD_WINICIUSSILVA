# LH_CD_WINICIUSSILVA
 Desafio Processo Seletivo - Lighthouse Programa De Formação Em Dados
...
*	O preço médio por noite aumenta com o aumento do número de meses que o anúncio está disponível, exceto no 11º mês. O preço médio por noite varia de 136,64 dólares no primeiro mês a 197,64 dólares no 12º mês, mas apresentando uma queda para 152,23 dólares no 11º mês. Isso pode indicar que os proprietários ajustam os seus preços de acordo com a sazonalidade e a demanda. Por exemplo, os preços podem ser mais altos nos meses de verão e de inverno, quando há mais turistas na cidade, e mais baixos no 11º mês, que corresponde a novembro, quando há menos procura.
...
*	Em seguida, quatro modelos de regressão diferentes foram testados: Regressão Linear Múltipla, LightGBM, Regressão com Random Forest e Regressão com Vetores de Suporte. A regressão é um tipo de problema que tenta prever um valor contínuo, como o preço de alugueis de imóveis, a partir de outras variáveis.
...
*	Dos modelos de regressão testados, os que obteve o melhores resultados foram o Random Forest, que usa várias árvores de decisão para fazer as previsões. Esse modelo teve um MAPE de 28.55%, um R² de 0.55, um MSE de 2797.53 e um RMSE de 52.89. O modelo LightGBM, que também é baseado em árvores de decisão e que utiliza uma técnica chamada de gradient boosting, teve um MAPE de 28.49%, um R² de 0.55, um MSE de 2768.90 e um RMSE de 52.62. Ambos os modelos tiveram resultados semelhantes, sendo considerados um empate técnico, mas eu escolhi usar o XGBoost por ter um tempo menor de treinamento.
...
*	Em conclusão, o LightGBM e Random Forest são bons modelos para prever o preço dos imóveis em Nova York, mas ainda tinha espaço para melhoria, especialmente para previsões de valores mais altos.


* Prós do modelo LightGBM:
...
```
git clone https://github.com/WiiniSilva/LH_CD_WINICIUSSILVA.git
```
...
with open('lgbm_model.pkl', 'rb') as file:
  lgbm_r = pickle.load(file)
```

2. Isso vai atribuir o seu modelo ao objeto lgbm_r, que você pode usar para fazer previsões ou avaliações, como se ele fosse o mesmo objeto original.